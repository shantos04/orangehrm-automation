...
test.describe("UI E2E Testing - OrangeHRM Login Module", () => {

    // ========================================================================
    // CẤU HÌNH TOÀN CỤC (GLOBAL)
    // LƯU Ý: Nhãn 'package' tự động đã được cấu hình trong login.fixture.ts
    // ========================================================================
    test.beforeEach(async () => {
        // Dịch cấp: Cấp 1 là UI E2E, Cấp 2 là Auth Module
    ...
    test.describe("Login Module - UI & Positive Flows @PositiveGroup", () => {
         ...
         */
        test.beforeEach(async () => {
            // Cấp 3: Ghi đè thành nhóm Positive
            await allure.subSuite("Positive Flows");
    ...
    test.describe("Login Module - Form Validation & Negative Flows @NegativeGroup", () => {
        ...
        test.beforeEach(async () => {
            // Cấp 3: Ghi đè thành nhóm Negative
            await allure.subSuite("Negative Validation");
            await allure.feature("Login Functionality");
        });
    ...
    test.describe("Login Module - Edge Cases & Data Handling @EdgeCaseGroup", () => {
        ...
        test.beforeEach(async () => {
            // Cấp 3: Ghi đè thành nhóm Edge Cases
            await allure.subSuite("Edge Cases & Data Handling");
            await allure.feature("Data Handling Constraints");
        });
        ...
        test("OrangeHRM_Login_TC11_VerifyFormReload @EdgeCase", async ({ page, loginPage }) => {
            ...
            await test.step("Verify: Username and Password fields are completely cleared", async () => {
                ...
                // Sửa lỗi typo ở file gốc của bạn (check username 2 lần) - Cập nhật thành txtPassword
    ...
    test.describe("Login Module - Security & Session Management @SecurityGroup", () => {
        ...
        test.beforeEach(async () => {
            // Cấp 3: Ghi đè thành nhóm Security Session
            await allure.subSuite("Security & Session Management");
            await allure.feature("Session and State Security");
        });
